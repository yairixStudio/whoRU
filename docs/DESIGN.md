...
TCC dialogs are drawn by `UserNotificationCenter` (`com.apple.UserNotificationCenter`). It does not run permanently; the system launches it when there is something to show. The Watcher therefore listens for `NSWorkspace.didLaunchApplicationNotification` filtered by bundle ID (and checks whether the process is already running at startup), then creates an `AXObserver` on its PID for `kAXWindowCreatedNotification` and `kAXUIElementDestroyedNotification`. A 300 ms poll of the process’s windows is the fallback while it is alive.
